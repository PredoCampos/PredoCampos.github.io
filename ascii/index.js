// --- Gerador de Arte ASCII - Versão JavaScript Pura ---
// Este script depende das bibliotecas gifuct-js e gif.js,
// que devem ser carregadas no index.html ANTES deste arquivo.
...
let currentAsciiData = null; // Armazena dados para downloads de imagem única
        ...
        img.onload = () => {
            ...
            if (isGif) {
                ...
                bufferReader.onload = (bufEvent) => {
                    ...
                    // As bibliotecas foram carregadas globalmente
...
async function downloadGif() {
    ...
    // A biblioteca GIF é carregada globalmente
    const gif = new GIF({
        workers: 2,
        quality: 10,
...
function handleDownloadSizeChange() {
    ...
    if (originalAspectRatio >= 1) { // Paisagem ou quadrado
        newWidth = size;
...
// Função auto-executável para iniciar os listeners
(async () => {
    SORTED_ASCII_CHARS = await analyzeAndSortChars(USER_ASCII_CHARS);
    charInputElement.value = USER_ASCII_CHARS.join('');
